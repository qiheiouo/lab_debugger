...
尚需强化：后台/缓存索引、超长 Session 压力测试、Session 浏览器，以及回放结构化结果而不重新解析的可选模式。手动 Marker 与阈值告警时间线已在 0.18 补齐。
...
- Phase 8 已完成 Marker 与基础告警：人工说明、曲线标注、阈值高/低条件、回差锁存、配置冻结、事件持久化、安全回放和端到端测试。
- Phase 8 已完成逐来源解析配置：默认继承、串口/网络独立 CSV 或二进制协议、解析状态隔离、Session 配置冻结、安全回放和旧 Session 兼容。
- Phase 8 已完成同类型动态数据源：最多 16 个本地串口/网络实例、运行期增删、精确发送目标、逐源状态与配置、Session 来源集合冻结，以及双 UDP 端到端回归。
- Phase 8 已完成持续阈值与运行健康告警：持续时间条件、滚动窗口错误频率、本地来源/ROS2 Topic 无数据超时、精确来源隔离、Session 配置冻结、安全回放和 UDP 端到端回归。
- Phase 8 已完成统一时间轴基础：逐来源源时间/电脑接收时间/手动偏移/ROS Agent 自动校正，规则继承、原始时间证据、延迟与不确定度、回退与乱序统计、Session 冻结及确定性回放。
- 主线在 0.22 形成阶段性稳定点，后续功能开发暂时冻结。恢复后优先完成真实硬件/跨机压力测试、完整使用教程和发布打包，再考虑组合告警、严格公共时间轴插值、TLS 与更高级曲线交互。
