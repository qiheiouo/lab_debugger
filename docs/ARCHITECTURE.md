...
`ThresholdAlertEngine` 同样位于纯核心层。规则只按完整字段名匹配有限数值，以高于/低于阈值和可选持续时间进入告警，并用非负回差控制复位；乱序样本不会推进计时或污染状态。`HealthAlertEngine` 则按精确 `sourceId` 维护滚动错误窗口和无数据计时，串口/网络打开及 ROS2 Topic 订阅会显式布防，新数据恢复后才重新允许超时触发。两个引擎都用锁保护跨线程状态，配置限制为 128 条并采用事务式替换，无效配置不会破坏当前有效规则。应用层把人工 Marker 和触发结果统一转换为 `SessionEvent`，实时绘图只消费只读快照；历史回放直接恢复事件，不重新执行告警，从而避免倍速与 seek 产生重复告警。

## 6. 生命周期与多数据源

应用层使用 `SourceManager` 动态管理 `SerialSource`、`NetworkSource` 与 `RemoteAgentSource`。桌面端可同时登记最多 16 个本地串口/网络实例；每个实例可独立连接、断开、重连和移除，终端按完整 `sourceId` 区分，流解析器互相隔离，发送入口也按完整标识精确路由。状态栏显示聚合流量，Session 在开始时冻结并记录全部已连接源。记录期间允许已声明源按原配置断开和重连，但拒绝新增、移除或改变来源，避免原始记录出现元数据未声明的数据。`ReplaySource` 与实时源互斥，打开回放会先关闭全部实时源并复用相同的显示、解析和曲线边界。

派生配置、数值告警、运行健康规则与源集合分别冻结到 `configuration/derived_fields.json`、`configuration/alert_rules.json`、`configuration/health_alert_rules.json` 和逐源配置。开始记录、打开回放及跳转都会重置派生状态与告警锁存状态；迟到且未声明的数据源可以继续显示，但不能改变已冻结 Session 的派生结果或写入健康告警。普通回放从原始数据重新计算，结构化 rosbag2 则以同一条消息的映射字段作为一个批次计算；Marker 和已发生告警直接从事件日志恢复。当前跨来源表达式采用最近有效值语义，严格的公共时间轴插值仍属于后续同步层。
