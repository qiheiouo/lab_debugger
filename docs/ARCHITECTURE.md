...
`DerivedFieldEngine` 位于纯核心层，使用受限 AST 解析四则运算、括号、常量和固定白名单数学函数，不执行任意脚本。配置阶段限制表达式长度、节点数和嵌套深度，并拒绝重复名、保留名及派生字段循环；配置失败不会替换当前有效定义。运行阶段只保存有限数值，按拓扑顺序计算受影响字段，任何缺失依赖、数学定义域错误或非有限结果都会让对应输出和下游失效。来源限定字段通过反引号精确引用，避免多源同名字段被猜测合并。
...
派生配置与源集合同时冻结到 `configuration/derived_fields.json`。开始记录、打开回放及跳转都会重置派生引擎的最近值；迟到且未声明的数据源可以继续显示，但不能改变已冻结 Session 的派生结果。普通回放从原始数据重新计算，结构化 rosbag2 则以同一条消息的映射字段作为一个批次计算。当前跨来源表达式采用最近有效值语义，严格的公共时间轴插值仍属于后续同步层。
...
- 派生变量目前提供无状态算术和基础数学函数；低通、高通、移动平均、微分、积分、角度展开、运行期错误计数和严格多源时间对齐仍待后续滤波/同步阶段实现。
