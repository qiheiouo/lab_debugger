...
- 后台 rosbag2 SQLite 导入、分卷时间归并、Topic/类型目录和原始 CDR 安全回放；
...
同一页面可直接导入单个 rosbag2 `.db3` 文件或包含多个分卷的 bag 目录。导入任务在后台执行，成功后自动打开标准 Session；ROS CDR 只进入原始终端和回放，不会被当作 CSV。详细规则见 [rosbag2 导入说明](docs/ROSBAG2.md)。
